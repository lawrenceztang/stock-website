...
def get_stocks(stock, year):

    file_name = "historical_stock_prices.csv"
    DAYS_AFTER_INITIAL = 30

    if not os.path.isfile("stocks/" + stock):
    ...
    stocks = []
    with open("stocks/" + stock) as file:
        reader = csv.DictReader(file)
        count = 0
        for i in reader:
            if count >= DAYS_AFTER_INITIAL and str_year_greater_eq_than(i["date"], year):
                stocks.append(i)
            count += 1
...
thirty_day_dataset = []
THIRTY_DAY_FILE_NAME = "30-day-stocks.csv"
def make_dataset():
...
def stock_sim():
    ...
    #check is valid - TODO
    return render_template("stock_game.html", ticker=ticker, input_from_python=get_stocks(request.form["ticker"], int(year)))
